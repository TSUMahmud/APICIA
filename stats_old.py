...
def connected(classfile, all_minus_classfile, flag, impacted_classfiles):
	if len(all_minus_classfile) == 0:
		...
		for f in all_minus_classfile:
			...
			if f_class in open(classfile, 'r').read() and f_class in impacted_classfiles:
				return True;
			else:
				remaining_classes = all_minus_classfile[1:];
				return flag or connected(f, remaining_classes, False, impacted_classfiles);
...
def get_stats(dict, files):
	...
	for cms in methods:
		for corr_class, mmethods in cms.items():
			...
			mmethods = [re.sub(r'\([^)]*\)', '', mmethod) + "()" for mmethod in mmethods];
	...
	def find_relevant_class(files, method):
		for f in files:
			...
			for d in data.split('\n'):
				...
				if (method in d) and ( 'public' in d or 'private' in d or 'static' in d or 'protected' in d):
...
def testmaps(testfiles, userclasses, usermethods):
	...
	# print (len(testfiles))
	aff_tests = [];
	testmap = {}; #testmap to impacted classes
	for f in testfiles:
		handle = open(f, 'r');
		k = f.split('/')[-1].split('.')[0];
...
def save_sep_methods(files, new_dir=0):
    ...
    for file in files:
        ...
        content = remove_comments(content);
        check = re.findall(r'(public|private|static) ([A-Za-z0-9<>.\[\]]+) ([A-Za-z0-9]+)\(', content)
